...
export const b64ToSymbols = str => str.split('').map(ch => symbolMap[b64chars.indexOf(ch)%symbolMap.length]).join('');
export const symbolsToB64 = str => str.split('').map(s => { 
...
export async function decryptMessage(symbolStr, passphrase) {
  try {
